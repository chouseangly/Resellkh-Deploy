...
const LoadingState = () => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-500 mb-4"></div>
    <p className="text-gray-600">Getting your location and fetching nearby products...</p>
  </div>
);
...
const LocationErrorState = ({ error, onRetry }) => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    ...
    <h2 className="font-semibold text-xl mb-2 text-red-600">Location Error</h2>
    <p className="text-gray-600 mb-4 max-w-md">{error}</p>
...
// ... (ApiErrorState and EmptyState remain the same)
...
export default function ListingNearMeClient() {
    ...
    return new Promise((resolve, reject) => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              errorMessage = "Location access denied. Please allow location permissions in your browser settings and refresh.";
              break;
            case error.POSITION_UNAVAILABLE:
        ...
        {
          // Change: enableHighAccuracy to false to avoid GPS timeouts on desktops
          enableHighAccuracy: false, 
          // Change: increase timeout to 15 seconds
          timeout: 15000, 
          maximumAge: 60000, 
        }
      );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes("location") || err.message.includes("Location") || err.message.includes("Geolocation")) {
        setLocationError(err.message);
      } else {
        setError("An unexpected error occurred while fetching products.");
      }
  ...
  const transformProduct = (product) => {
    ...
    return {
      ...
      description: product.description,
      price: price.toFixed(2),
      originalPrice: product.discountPercent > 0 ? product.productPrice.toFixed(2) : null,
      discountText: product.discountPercent > 0 ? `${product.discountPercent}% OFF` : null,
    };
  };

  const renderContent = () => {
    if (loading) return <LoadingState />;
    if (locationError) return <LocationErrorState error={locationError} onRetry={fetchData} />;
    if (error) return <ApiErrorState error={error} onRetry={fetchData} />;
    if (products.length === 0) return <EmptyState />;

    return (
      <div className="grid grid-cols-2 sm:grid-cols-2 px-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 justify-items-center">
        {products.map((product) => (
          <ProductCart key={product.productId} {...transformProduct(product)} />
        ))}
      </div>
    );
  };
  ...
  const validProducts = products.filter((p) => isFinite(p.distance));
  const maxDistance = validProducts.length > 0
      ? Math.ceil(validProducts[validProducts.length - 1].distance)
      : 0;
  ...
  return (
    <>
      <div className="flex ps-[7%] mt-3 items-center">
        ...
        <svg className="mx-1" width="20" height="20" viewBox="0 0 21" fill="none"><path fillRule="evenodd" clipRule="evenodd" d="M6.98558 5.06864C7.32339 4.7931 7.8211 4.8128 8.13643 5.12775L13.0048 9.99638C13.1679 10.1596 13.2563 10.3779 13.2563 10.6044C13.2563 10.8309 13.1681 11.0488 13.0048 11.2127L8.13633 16.0811C7.80004 16.417 7.2557 16.417 6.92029 16.0811C6.58388 15.7451 6.58388 15.2006 6.92019 14.8648L11.1802 10.6044L6.92029 6.34407C6.60492 6.02908 6.5852 5.53088 6.86112 5.19302L6.92025 5.12769L6.98558 5.06864Z" fill="#343A40"/></svg>
        <span className="pointer-events-none text-orange-500">Listing near me</span>
      </div>
      ...
      <div className="px-[7%]">
        <div className="p-2 mt-2 md:p-6">
          <div className="p-3 rounded-[24px] border border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <h2 className="text-sm md:text-lg lg:text-lg font-semibold text-gray-800">
                {loading ? "Finding listings..." : `${products.length} listings found near you`}
              </h2>
              <div className="flex items-center gap-2 text-gray-600 text-sm">
                <span>{loading || !validProducts.length ? "Sorted by distance" : `Results up to ~${maxDistance} km away`}</span>
              </div>
            </div>
            {renderContent()}
          </div>

...
const LoadingState = () => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-500 mb-4"></div>
    <p className="text-gray-600">Getting high-accuracy location and fetching products...</p>
  </div>
);
...
const ApiErrorState = ({ error, onRetry }) => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    ...
    <p className="text-gray-600 mb-4">{error}</p>
    <button onClick={onRetry} className="bg-orange-500 text-white px-6 py-2 rounded-lg hover:bg-orange-600 transition-colors">Retry</button>
  </div>
);
...
export default function ListingNearMeClient() {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          if (error.code === error.PERMISSION_DENIED) {
            errorMessage = "Please grant location access in your browser settings and try again.";
          }
          reject(new Error(errorMessage));
        },
        {
          enableHighAccuracy: true,
          timeout: 15000,
          maximumAge: 0, // ⚡ FIX: Force fresh location (improves Laptop accuracy)
        }
      );
    });
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // ⚡ FIX: Check URL params first (allows manual testing with your phone's URL)
      ...
      const nearbyProducts = await fetchNearbyProducts(lat, lng);

      const productsWithDistance = nearbyProducts
        .map((product) => {
          if (typeof product.latitude === "number" && typeof product.longitude === "number") {
            const distance = getDistanceFromLatLonInKm(lat, lng, product.latitude, product.longitude);
            return { ...product, distance };
          }
          return { ...product, distance: Infinity };
        })
      ...
      if (err.message.includes("location") || err.message.includes("Permission")) {
        ...
        setError("Could not fetch products for this location.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const transformProduct = (product) => {
    const price = typeof product.productPrice === "number" && typeof product.discountPercent === "number"
        ? (product.productPrice * (100 - product.discountPercent)) / 100
        : product.productPrice || 0;
  ...
  return (
    <>
      <div className="flex ps-[7%] mt-3 items-center">
        <p className="text-gray-500"><Link href="/">Home</Link></p>
        <span className="mx-2 text-gray-400">/</span>
        <span className="text-orange-500">Listing near me</span>
      </div>
          ...
          <div className="p-3 rounded-[24px] border border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <h2 className="text-sm md:text-lg font-semibold text-gray-800">
                {loading ? "Finding listings..." : `${products.length} listings found`}
              </h2>
              <span className="text-gray-600 text-sm">
                {loading || !validProducts.length ? "Detecting location..." : `Results within ~${maxDistance} km`}
              </span>
            </div>
            {loading ? <LoadingState /> : locationError ? <LocationErrorState error={locationError} onRetry={fetchData} /> : error ? <ApiErrorState error={error} onRetry={fetchData} /> : products.length === 0 ? <EmptyState /> : (
